class Game {
    constructor(cardsSection, cardsCount) {
        this.cardsHolder = cardsSection;
    ...
    flip(selectedCard) {
        ...
        if (flippedCards.length === 2) {
            this.cardsHolder.classList.add('pause_game');

            setTimeout(() => {
                this.cardsHolder.classList.remove('pause_game');
    ...
    generateField() {
        ...
        while(this.cardsHolder.children.length < this.cardsCount) {
            ...
            for (let j = 0; j < 2; j++) {
                this.cardsHolder.insertAdjacentHTML('beforeend', `
...
<div class="back_card">${questionIcon}</div>
        ...
        this.cards = Array.from(this.cardsHolder.children);
